...
async function generatePdf(signals, scoring, languages) {
    const [{ jsPDF }, autoTableMod] = await Promise.all([
        ...
        Promise.resolve().then(() => __importStar(require("jspdf-autotable"))),
    ]);
    ...
    const proj = (0, scanners_export_1.buildProjectScanSummary)(0, rawSignals, factorsPayload, languages || {});
    const doc = new jsPDF({ orientation: "landscape", unit: "pt", format: "a4" });
    const pageWidth = doc.internal.pageSize.getWidth();
    const pageHeight = doc.internal.pageSize.getHeight();
    const projectTitle = proj.project_path || `project #${proj.project_id}`;
    const attributes = scoring.attributes ? [...scoring.attributes] : [];
    ...
    for (let i = 0; i < DIMENSIONS.length; i += 1) {
        ...
        drawDimensionCard(doc, cardsX + i * (cardW + 8), heroY, cardW, heroH, dim.label, score ?? null);
    }
    // File census strip
    ...
    if (proj.languages && Object.keys(proj.languages).length > 0) {
        ...
        const totalLangFiles = Object.values(proj.languages || {}).reduce((s, n) => s + (typeof n === "number" ? n : 0), 0);
        const top = Object.entries(proj.languages || {})
            .sort((a, b) => (b[1] || 0) - (a[1] || 0))
            .slice(0, 4)
            .map(([k, v]) => {
            const pct = totalLangFiles > 0 ? Math.round((v / totalLangFiles) * 100) : 0;
            return `${k} ${pct}%`;
        });
        doc.setFont("helvetica", "bold");
        doc.setFontSize(9);
    ...
    const issues = topN(sorted.filter((a) => a.delta_to_score < -0.05), 5, (a, b) => a.delta_to_score - b.delta_to_score);
    const strengths = topN(sorted.filter((a) => a.delta_to_score > 0.05), 5, (a, b) => b.delta_to_score - a.delta_to_score);
    const hasLangs = !!proj.languages && Object.keys(proj.languages).length > 0;
    ...
    if (sorted.length === 0) {
        doc.setFontSize(11);
        doc.setTextColor(100, 116, 139);
        doc.text("No scanner attributes captured for this project's latest scan.", 40, cursor + 14);
        doc.setTextColor(0);
        cursor += 28;
    }
    // ── Methodology card ─
    ...
    for (const cat of orderedCategories) {
        ...
        doc.setFontSize(9);
        doc.setTextColor(100, 116, 139);
        doc.text(`${items.length} finding${items.length === 1 ? "" : "s"}  ·  Net score impact ${totalDelta >= 0 ? "+" : ""}${totalDelta.toFixed(1)}`, 40, cursor + 30);
        doc.setTextColor(0);
        cursor += 42;
        ...
        const scanners = SCANNER_ORDER.filter((s) => byScanner.has(s)).concat(Array.from(byScanner.keys()).filter((s) => !SCANNER_ORDER.includes(s)));
...
function expandEvidence(evidence, opts = {}) {
    ...
    if (typeof evidence === "string") {
        ...
        try {
            ...
            if (typeof parsed !== "string")
                return expandEvidence(parsed, opts);
        }
        catch {
            /**/
        }
...
function summariseEvidence(evidence, maxItems = 12) {
    if (evidence == null)
        return "";
    if (typeof evidence === "string") {
        if (!evidence)
        ...
        try {
            const parsed = JSON.parse(evidence);
            if (typeof parsed !== "string")
                return summariseEvidence(parsed, maxItems);
        }
        catch {
            /**/
        }
...
function effectiveSeverity(scanner, attributeKey, delta) {
    if (delta > 0.05)
        return { label: "STRENGTH", tone: "good" };
    const hint = (0, scanner_fix_hints_1.getFixHint)(scanner, attributeKey);
...
function drawHealthHero(doc, x, y, w, h, health, grade) {
...
function drawDimensionCard(doc, x, y, w, h, label, score) {
...
function drawMethodologyBox(doc, x, y, width) {
    const pad = 12;
    const lineHeight = 11;
...
function setFooter(doc, pageWidth, pageHeight, projectTitle) {
...
function startSectionPage(doc, pageWidth, title, subtitle) {
    doc.addPage();
    doc.setFillColor(248, 250, 252);
...
function drawSubSectionTitle(doc, x, y, title) {
    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    doc.setTextColor(51, 65, 85);
    doc.text(title, x, y);
...
function trimEvidence(s, maxChars = 220) {
    if (s.length <= maxChars)
        return s;
    return s.slice(0, maxChars - 1).trimEnd() + "…";
}
function renderProjectDetailsPage(doc, autoTable, proj, pageWidth, pageHeight, projectTitle) {
...
function renderArchitecturePage(doc, autoTable, arch, pageWidth, pageHeight, projectTitle) {
    if (!arch.has_doc)
        return;
...
function renderModulesPage(doc, autoTable, modules, frameworks, pageWidth, pageHeight, projectTitle) {
...
function renderDocsPage(doc, autoTable, docs, pageWidth, pageHeight, projectTitle) {
    if (docs.total_md_files === 0 && !docs.has_docs_dir && !docs.has_architecture_doc && !docs.has_api_doc)
        return;
    ...
    cursor = lastTableY(doc, cursor) + 18;
    const sectionLabels = [["setup", "Setup / Getting started"], ["usage", "Usage / Examples"], ["test", "Testing"], ["deploy", "Deploy / Operations"], ["api", "API reference"], ["architecture", "Architecture"], ["contributing", "Contributing"], ["changelog", "Changelog"], ["troubleshooting", "Troubleshooting"], ["faq", "FAQ"]];
    if (cursor > pageHeight - 140) {
        doc.addPage();
        cursor = 40;
    }
    cursor = drawSubSectionTitle(doc, 40, cursor, "Sections checklist");
    autoTable(doc, { ...KV_TABLE_STYLES, startY: cursor, margin: { left: 40, right: 40 }, head: [["Section", "Present"]], body: sectionLabels.map(([k, label]) => [label, docs.sections[k] ? "Yes" : "No"]), columnStyles: { 0: { cellWidth: 240 }, 1: { cellWidth: 100, halign: "center" } }, didDrawPage: () => setFooter(doc, pageWidth, pageHeight, projectTitle), didParseCell: (data) => { if (data.section !== "body")
            return; if (data.column.index === 1) {
            applyTone(data, String(data.cell.raw) === "Yes" ? "good" : "warn");
        } } });
}
function renderVulnsPage(doc, autoTable, vulns, pageWidth, pageHeight, projectTitle) {
    if (vulns.total_resolved === 0 && vulns.findings.length === 0 && vulns.lockfiles.length === 0)
        return;
    ...
    autoTable(doc, { ...KV_TABLE_STYLES, startY: cursor, margin: { left: 40, right: 40 }, head: [["Severity", "Count"]], body: [["Critical", String(vulns.totals.critical)], ["High", String(vulns.totals.high)], ["Medium", String(vulns.totals.medium)], ["Low", String(vulns.totals.low)]], columnStyles: { 0: { cellWidth: 160 }, 1: { cellWidth: 120, halign: "right" } }, didDrawPage: () => setFooter(doc, pageWidth, pageHeight, projectTitle), didParseCell: (data) => { if (data.section !== "body" || data.column.index !== 0)
            return; applyTone(data, vulnSeverityTone(String(data.cell.raw).toLowerCase())); } });
    cursor = lastTableY(doc, cursor) + 18;
    ...
    if (vulns.findings.length > 0) {
        ...
        autoTable(doc, { ...KV_TABLE_STYLES, startY: cursor, margin: { left: 40, right: 40 }, head: [["Severity", "Package", "Version", "Ecosystem", "Advisory", "Range", "Summary"]], body: vulns.findings.map((f) => [f.advisory.severity, f.package, f.version, f.ecosystem, f.advisory.id, f.advisory.range, f.advisory.summary]), columnStyles: { 0: { cellWidth: 60, halign: "center" }, 1: { cellWidth: 130 }, 2: { cellWidth: 70 }, 3: { cellWidth: 70 }, 4: { cellWidth: 110 }, 5: { cellWidth: 90 }, 6: { cellWidth: "auto" } }, didDrawPage: () => setFooter(doc, pageWidth, pageHeight, projectTitle), didParseCell: (data) => { if (data.section !== "body" || data.column.index !== 0)
                return; applyTone(data, vulnSeverityTone(String(data.cell.raw).toLowerCase())); } });
    }
}
function renderContentFindingsPage(doc, autoTable, content, pageWidth, pageHeight, projectTitle) {
    if (content.hits.length === 0 && content.longest_files.length === 0 && content.totals.critical === 0 && content.totals.warning === 0 && content.totals.suggestion === 0)
        return;
    ...
    if (content.hits.length > 0) {
        ...
        autoTable(doc, { ...KV_TABLE_STYLES, startY: cursor, margin: { left: 40, right: 40 }, head: [["Severity", "File", "Line", "Title", "Evidence"]], body: content.hits.map((h) => [h.severity, h.file, String(h.line), h.title, trimEvidence(h.evidence, 300)]), styles: { fontSize: 8.5, cellPadding: 4, overflow: "linebreak", valign: "top" }, columnStyles: { 0: { cellWidth: 60, halign: "center" }, 1: { cellWidth: 200 }, 2: { cellWidth: 40, halign: "right" }, 3: { cellWidth: 200 }, 4: { cellWidth: "auto" } }, didDrawPage: () => setFooter(doc, pageWidth, pageHeight, projectTitle), didParseCell: (data) => { if (data.section !== "body" || data.column.index !== 0)
                return; const sev = String(data.cell.raw).toLowerCase(); applyTone(data, sev === "critical" ? "bad" : sev === "warning" ? "warn" : "info"); } });
        cursor = lastTableY(doc, cursor) + 18;
    }
...
function renderAstPage(doc, autoTable, ast, pageWidth, pageHeight, projectTitle) {
    if (ast.total_functions === 0)
        return;
    ...
    if (fns.length > 0) {
        ...
        autoTable(doc, { ...KV_TABLE_STYLES, startY: cursor, margin: { left: 40, right: 40 }, head: [["Cx", "Function", "File", "Lines", "LOC", "Params", "Nest", "Flags"]], body: ranked.map((fn) => { const flags = []; if (fn.is_exported)
                flags.push("export"); if (fn.is_exported && !fn.has_doc_comment)
                flags.push("no docs"); if (fn.is_untested && fn.is_exported && fn.complexity >= 10)
                flags.push("untested"); return [String(fn.complexity), fn.name, fn.file, `${fn.start_line}-${fn.end_line}`, String(fn.loc), String(fn.params), String(fn.max_nesting), flags.join(", ") || "—"]; }), styles: { fontSize: 8.5, cellPadding: 4, overflow: "linebreak", valign: "top" }, columnStyles: { 0: { cellWidth: 40, halign: "right" }, 1: { cellWidth: 160 }, 2: { cellWidth: 220 }, 3: { cellWidth: 70, halign: "right" }, 4: { cellWidth: 50, halign: "right" }, 5: { cellWidth: 50, halign: "right" }, 6: { cellWidth: 40, halign: "right" }, 7: { cellWidth: "auto" } }, didDrawPage: () => setFooter(doc, pageWidth, pageHeight, projectTitle), didParseCell: (data) => { if (data.section !== "body" || data.column.index !== 0)
                return; const cx = parseFloat(String(data.cell.raw)); if (Number.isFinite(cx)) {
                applyTone(data, cx >= 20 ? "bad" : cx >= 15 ? "warn" : "info");
            } } });
    }
}
function renderFactorBreakdownPage(doc, autoTable, breakdown, pageWidth, pageHeight, projectTitle) {
    if (!breakdown)
        return;
    const dims = Object.keys(breakdown).filter((k) => !!breakdown[k]);
    if (dims.length === 0)
        return;
    ...
    for (const k of dims) {
        ...
        if (!dim || !dim.factors)
            continue;
        const label = (DIMENSIONS.find((d) => d.key === k)?.label ?? String(k));
        if (cursor > pageHeight - 160) {
            doc.addPage();
            cursor = 40;
        }
