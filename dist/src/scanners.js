...
function getGitInfo(rootDir) {
    try {
        ...
        const headContent = node_fs_1.default.readFileSync(headPath, "utf-8").trim();
        const parts = headContent.split("refs/heads/");
        const branch = headContent.includes("ref:") && parts[1] ? parts[1] : null;
        const commitPath = branch ? node_path_1.default.join(rootDir, `.git/refs/heads/${branch}`) : node_path_1.default.join(rootDir, ".git/HEAD");
        let commit = null;
        if (node_fs_1.default.existsSync(commitPath)) {
            commit = node_fs_1.default.readFileSync(commitPath, "utf-8").trim().substring(0, 8);
        }
        let origin = null;
        try {
            const configPath = node_path_1.default.join(rootDir, ".git/config");
...
function runScanners(repo, projectName) {
    ...
    const hasFiles = (patterns) => repo.files.some((f) => patterns.some((p) => f.includes(p)));
    ...
    return {
        ...
        test_to_source_ratio: fileStats.sourceFiles > 0
        ...
        has_secret_files: repo.files.filter((f) => /\.(pem|key|secret|credentials)$/i.test(f)),
...
function analyzeFiles(files) {
    ...
    for (const file of files) {
        ...
        const depth = file.split(node_path_1.default.sep).length;
        if (depth > 10)
            deeplyNestedCount = deeplyNestedCount + 1;
        const relativePath = file;
        if (/\.(test|spec)\.(ts|tsx|js|jsx|py)$/.test(relativePath)) {
            testFiles++;
        }
        else if (/\.(ts|tsx|js|jsx|py|go|rs|java|cpp|c)$/.test(relativePath) &&
...
function detectLanguages(fileList) {
    const langs = {};
    const langMap = {
        ".ts": "TypeScript",
        ".tsx": "TSX",
...
function detectFrameworks(rootDir, _files) {
...
function checkTsConfigStrict(rootDir) {
    try {
        const tsconfigPath = node_path_1.default.join(rootDir, "tsconfig.json");
        if (!node_fs_1.default.existsSync(tsconfigPath))
            return false;
        const content = node_fs_1.default.readFileSync(tsconfigPath, "utf-8");
...
function checkTsConfigNoUnchecked(rootDir) {
    try {
        const tsconfigPath = node_path_1.default.join(rootDir, "tsconfig.json");
        if (!node_fs_1.default.existsSync(tsconfigPath))
            return false;
...
function scanDatabaseSchema(repo) {
    try {
        const schemaSignals = (0, repo_schema_scan_1.scanRepoSchema)(repo);
        const findings = [];
        const strengths = [];
        ...
        if (schemaSignals.models_found > 0) {
            const indexed = schemaSignals.models.filter((m) => m.has_non_pk_index).length;
...
function scanLayering(repo) {
    try {
        const layeringSignals = (0, repo_layering_scan_1.scanRepoLayering)(repo);
        const findings = [];
...
function scanTestMap(repo) {
    try {
        const testMapSignals = (0, repo_test_map_scan_1.scanRepoTestMap)(repo);
        const findings = [];
...
function scanRoutes(repo) {
    try {
        const routeSignals = (0, repo_route_scan_1.scanRepoRoutes)(repo);
        const findings = [];
        ...
        if (routeSignals.routes_without_validate && routeSignals.routes_without_validate > 0) {
            const writeRoutes = routeSignals.routes.filter((r) => ['POST', 'PUT', 'PATCH', 'DELETE'].includes(r.method));
        ...
        return {
            ...
            write_endpoints: routeSignals.routes.filter((r) => ['POST', 'PUT', 'PATCH', 'DELETE'].includes(r.method)).length || 0,
...
function scanVulnerabilities(rootDir) {
    const lockfiles = [];
    const findings = [];
    try {
        if (node_fs_1.default.existsSync(node_path_1.default.join(rootDir, "package-lock.json"))) {
            const pkg = JSON.parse(node_fs_1.default.readFileSync(node_path_1.default.join(rootDir, "package-lock.json"), "utf-8"));
        ...
        if (node_fs_1.default.existsSync(node_path_1.default.join(rootDir, "yarn.lock"))) {
            lockfiles.push({
                ecosystem: "yarn",
                lockfile: "yarn.lock",
        ...
        if (node_fs_1.default.existsSync(node_path_1.default.join(rootDir, "pnpm-lock.yaml"))) {
            lockfiles.push({
                ecosystem: "pnpm",
...
function scanDocsReal(repo) {
    try {
        return (0, repo_doc_scan_1.scanRepoDocs)(repo);
    }
    catch (e) {
        return {
            total_md_files: 0,
...
function scanDocumentation(rootDir, files) {
    ...
    try {
        for (const file of docFiles.slice(0, 20)) {
            const fullPath = node_path_1.default.join(rootDir, file);
...
function scanArchitectureReal(repo) {
    try {
        return (0, repo_architecture_scan_1.scanRepoArchitecture)(repo);
    }
    catch (e) {
        return {
            has_doc: false,
...
function scanArchitecture(rootDir) {
    const docFiles = require("fs")
        .readdirSync(rootDir, { recursive: true })
        .filter((f) => /architecture|arch/i.test(f.toLowerCase()) && f.endsWith(".md"));
    return {
...
function scanModulesReal(repo) {
    // Module detection - keep synthetic for now as mr-analyzer doesn't export this separately
    try {
        const srcModules = repo.files.filter((f) => f.startsWith("src/")).length > 0;
        return {
        ...
        return { length: 0, map: (fn) => [] };
    }
}
function scanModules(rootDir, files) {
...
function scanContentReal(repo) {
    try {
        return (0, repo_content_rules_1.scanRepoContent)(repo);
    }
    catch (e) {
        return {
            totals: { by_rule: {} },
...
function scanContent(files) {
    const sourceFiles = files.filter((f) => /\.(ts|tsx|js|jsx|py|go|rs|java|cpp|c|rb|php)$/.test(f) &&
    ...
    try {
        for (const file of sourceFiles) {
            const fullPath = node_path_1.default.join(process.cwd(), file);
            if (!node_fs_1.default.existsSync(fullPath))
                continue;
            const content = node_fs_1.default.readFileSync(fullPath, "utf-8");
...
function scanAstReal(repo) {
    try {
        return (0, repo_ast_scan_1.scanRepoAst)(repo);
    }
    catch (e) {
        return {
            total_functions: 0,
...
function scanAst(rootDir, files) {
        ...
        for (const file of sourceFiles) {
            ...
            for (const pattern of fnPatterns) {
                ...
                while ((match = pattern.exec(content)) !== null) {
                    ...
                    const cyclomatic = (fnBody.match(/\bif\b|\?:/g) || []).length +
    ...
    // Remove duplicates
    const uniqueFunctions = Array.from(new Map(functions.map((f) => [`${f.file}:${f.name}`, f])).values());
    ...
    const locMedian = functionLocs[Math.floor(functionLocs.length / 2)] || 0;
    ...
    return {
        ...
        untested_complex_functions: uniqueFunctions.filter((f) => f.is_untested && f.complexity >= 10).length,
        functions: uniqueFunctions
            .sort((a, b) => b.complexity - a.complexity)
            .slice(0, 50),
    };
}
function scanDeps(rootDir) {
    return {
        vulnerable: [],
        outdated_hints: [],
    };
