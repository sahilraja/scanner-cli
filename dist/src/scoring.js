...
function computeScores(signals) {
    ...
    const avgDim = (dimensions.code_quality.score +
    ...
    const entries = Object.entries(dimensions).sort(([, a], [, b]) => a.score - b.score);
    ...
    return {
        ...
        grade,
        verdict,
        worst_dimension: (entries[0]?.[0] || 'code_quality'),
        factor_breakdown: dimensions,
        warnings: [],
        attributes,
    };
}
function computeCodeQuality(signals) {
...
function computeSecurity(signals) {
...
function computePerformance(signals) {
...
function computeTestCoverage(signals) {
...
function computeReadability(signals) {
...
function generateAttributes(signals) {
    const attributes = [];
    ...
    if (signals.modules && signals.modules.length > 0) {
        attributes.push({
            ...
            delta_to_score: 0.2,
            evidence: signals.modules.map((m) => m.label).join(", "),
        });
    }
    // Architecture
    ...
    if (signals.vulns && signals.vulns.lockfiles && signals.vulns.lockfiles.length > 0) {
        attributes.push({
            ...
            delta_to_score: 0.1,
            evidence: signals.vulns.lockfiles.map((l) => l.lockfile).join(", "),
        });
    }
    ...
    if (signals.vulns && signals.vulns.findings && signals.vulns.findings.length > 0) {
        ...
        const criticalCount = signals.vulns.findings.filter((f) => f.advisory.severity === "critical").length;
        const highCount = signals.vulns.findings.filter((f) => f.advisory.severity === "high").length;
        const mediumCount = signals.vulns.findings.filter((f) => f.advisory.severity === "medium").length;
        if (criticalCount > 0) {
            attributes.push({
                category: "security",
